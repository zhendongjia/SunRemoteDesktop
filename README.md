...
- 登录前界面直接采用 RDP 客户端请求的桌面尺寸，并支持键盘、绝对/相对鼠标和 MS-RDPEI 单指触摸；认证后会列出这台机器主显示器实际支持的分辨率，默认选中当前物理分辨率，同时显示客户端分辨率并给出最接近客户端长宽比和尺寸的推荐模式。如果已有客户端占用物理控制台，后来登录的客户端会在同一页看到“断开其他客户端”选项，明确确认后才会接管。
- 会话代理可在 Windows 显示模式变化后继续传送新尺寸；缩放模式保持桌面长宽比并居中留边，绝对鼠标坐标按实际画面区域同步换算。
- 支持 RDP Display Control 动态分辨率：客户端窗口变化会重建 RDP 画布，但不会擅自改变用户已经选定的物理显示模式；页面上的推荐值会随客户端画布更新。选定模式与客户端画布不同且硬件没有精确模式时，画面保持长宽比并使用高质量锐化重采样居中适配；控制权在接管时转移，最后一个拥有控制权的 RDP 客户端断开时恢复首个连接前的本地显示模式。
...
认证界面可以用鼠标或客户端的直接触摸模式选择输入框和按钮，也可以使用 `Tab` 切换、`Enter` 提交。当前原生触摸支持单指点击和拖动，多点手势暂不转发。分辨率页默认选中当前物理屏模式；左右方向键或主卡片左右两侧可遍历主机实际支持的模式，`Space` 或推荐卡片可直接采用按客户端画布计算出的建议值，`Enter` 确认。勾选“Remember this resolution”后，服务只按已经通过 Windows 认证的账户保存这个具体主机分辨率，下次同一账户连接且该模式仍受支持时自动复用；它不使用来源 IP、路由器、代理或客户端自报名称，也不保存密码。已有客户端在线时，即使存在已记住的分辨率，后来连接仍必须明确勾选“断开其他客户端”（键盘可在对应选项按 `Space`）再按 `Enter`，服务才会断开旧会话并让新客户端接管。
...
认证探针要求所选客户端画布与物理屏尺寸不同；默认模式复测兼容登录页，增加 `--nla` 后复测 Windows App 使用的连接前 CredSSP 通路。两种模式都会在 Windows 验证身份后确认默认的当前物理分辨率，并断言访问页切换为真实桌面时连接仍然存活。已有另一探针或真实客户端在线时，`--takeover` 会额外确认“断开其他客户端”复选项。加上 `--post-auth-wait-seconds 8` 可在认证后持续消费桌面 Surface 更新，适合同时执行 `scripts/invoke-maintenance.ps1 -Action RestartAgent`，复测控制台代理交接期间 RDP 连接是否保持。它仍不能替代远程设备上的显示质量、网络延迟和本地可见输入检查。
