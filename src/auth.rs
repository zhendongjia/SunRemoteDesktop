...
impl CredentialValidator for LocalAccountValidator {
    async fn validate(
        ...
        if self.access_gate.has_trusted_sign_in() {
            tracing::info!("trusted RDP client reused its remembered sign-in");
...
pub fn is_account_allowed(config_path: &std::path::Path, account: &str) -> Result<bool> {
